...
export function createSessionStore(options: {
  ...
  const mayaConfigPattern = /[\\/]\.opencode[\\/]maya\.json\b/i;
  ...
  const detectReloadReason = (value: unknown): ReloadReason | null => {
    ...
    if (!text) return null;
    if (mayaConfigPattern.test(text)) return null;
  ...
  const detectReloadTriggerFromText = (text: string): ReloadTrigger | null => {
    if (mayaConfigPattern.test(text)) {
      return null;
    }
  ...
  const invalidToolNextStepHint = (part: Part) => {
    ...
    if (lower.includes("browser") || lower.includes("chrome") || lower.includes("devtools")) {
      return "MAYA browser automation isn't set up yet. Go to Plugins and ensure the browser plugin/extension is installed and connected, then retry.";
    }
