...
export default function AppEntry() {
  const defaultUrl = (() => {
    ...
    // When running the web UI against an MAYA server (e.g. Docker dev stack),
    ...
    const mayaUrl =
      typeof import.meta.env?.VITE_OPENWORK_URL === "string"
        ...
        : "";
    if (mayaUrl) {
      return `${mayaUrl.replace(/\/+$/, "")}/opencode`;
    }

    // When the UI is served by the MAYA server (Docker "remote" mode),
