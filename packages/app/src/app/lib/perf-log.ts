...
type PerfRoot = typeof globalThis & {
  __mayaPerfSeq?: number;
  __mayaPerfLogs?: PerfLogRecord[];
  __mayaPerfConsoleAt?: Record<string, number>;
  __mayaPerfConsoleSuppressed?: Record<string, number>;
};
...
export const recordPerfLog = (
  ...
  const id = (root.__mayaPerfSeq ?? 0) + 1;
  root.__mayaPerfSeq = id;
  ...
  const logs = root.__mayaPerfLogs ?? [];
  logs.push(entry);
  if (logs.length > PERF_LOG_LIMIT) {
    logs.splice(0, logs.length - PERF_LOG_LIMIT);
  }
  root.__mayaPerfLogs = logs;

  try {
    const key = `${scope}:${event}`;
    const now = Date.now();
    const lastByKey = root.__mayaPerfConsoleAt ?? (root.__mayaPerfConsoleAt = {});
    const suppressedByKey =
      root.__mayaPerfConsoleSuppressed ?? (root.__mayaPerfConsoleSuppressed = {});
