...
import { buildOpenworkWorkspaceBaseUrl, parseOpenworkWorkspaceIdFromUrl } from "../lib/maya-server";
import type { OpenworkServerSettings, OpenworkServerStatus } from "../lib/maya-server";
...
export default function ConfigView(props: ConfigViewProps) {
  const [mayaUrl, setOpenworkUrl] = createSignal("");
  const [mayaToken, setOpenworkToken] = createSignal("");
  const [mayaTokenVisible, setOpenworkTokenVisible] = createSignal(false);
  const [mayaTestState, setOpenworkTestState] = createSignal<"idle" | "testing" | "success" | "error">("idle");
  const [mayaTestMessage, setOpenworkTestMessage] = createSignal<string | null>(null);
  ...
  createEffect(() => {
    setOpenworkUrl(props.mayaServerSettings.urlOverride ?? "");
    setOpenworkToken(props.mayaServerSettings.token ?? "");
  });
  ...
  const reloadAvailabilityReason = createMemo(() => {
    ...
    if (!props.canReloadWorkspace) {
      return "Reloading is only available for local workers or connected MAYA servers.";
    }
    return null;
  });
  ...
  const buildOpenworkSettings = () => ({
    ...props.mayaServerSettings,
    urlOverride: mayaUrl().trim() || undefined,
    token: mayaToken().trim() || undefined,
  });

  const hasOpenworkChanges = createMemo(() => {
    const currentUrl = props.mayaServerSettings.urlOverride ?? "";
    const currentToken = props.mayaServerSettings.token ?? "";
    return mayaUrl().trim() !== currentUrl || mayaToken().trim() !== currentToken;
  });

  const resolvedWorkspaceId = createMemo(() => {
    const explicitId = props.mayaServerWorkspaceId?.trim() ?? "";
    if (explicitId) return explicitId;
    return parseOpenworkWorkspaceIdFromUrl(mayaUrl()) ?? "";
  });

  const resolvedWorkspaceUrl = createMemo(() => {
    const baseUrl = mayaUrl().trim();
    if (!baseUrl) return "";
    return buildOpenworkWorkspaceBaseUrl(baseUrl, resolvedWorkspaceId()) ?? baseUrl;
  });

  const hostInfo = createMemo(() => props.mayaServerHostInfo);
  ...
  const diagnosticsBundle = createMemo(() => {
    const urlOverride = props.mayaServerSettings.urlOverride?.trim() ?? "";
    const token = props.mayaServerSettings.token?.trim() ?? "";
    const host = hostInfo();
    ...
    return {
      ...
      workspace: {
        mayaServerWorkspaceId: props.mayaServerWorkspaceId ?? null,
        clientConnected: props.clientConnected,
        anyActiveRuns: props.anyActiveRuns,
      },
      mayaServer: {
        status: props.mayaServerStatus,
        url: props.mayaServerUrl,
        settings: {
  ...
  return (
    <section class="space-y-6">
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-2">
        ...
        <Show when={props.mayaServerWorkspaceId}>
          <div class="text-[11px] text-gray-7 font-mono truncate">
            Workspace: {props.mayaServerWorkspaceId}
          </div>
        </Show>
      </div>
      ...
      <Show when={hostInfo()}>
        <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
          <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <div class="text-sm font-medium text-gray-12">MAYA server sharing</div>
              <div class="text-xs text-gray-10">
          ...
          <div class="grid gap-3">
            <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
              <div class="min-w-0">
                <div class="text-xs font-medium text-gray-11">MAYA Server URL</div>
      ...
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <div class="text-xs text-gray-10">
              Connect to an MAYA server. Use the URL and access token from your server admin.
            </div>
          </div>
          <div class={`text-xs px-2 py-1 rounded-full border ${mayaStatusStyle()}`}>{mayaStatusLabel()}</div>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
          <Button
            variant="secondary"
            onClick={async () => {
              if (mayaTestState() === "testing") return;
            ...
            disabled={props.busy || mayaTestState() === "testing"}
          >
            {mayaTestState() === "testing" ? "Testing..." : "Test connection"}
          </Button>
          <Button
            variant="outline"
        ...
        <Show when={mayaTestState() !== "idle"}>
          <div
            class={`text-xs ${
              mayaTestState() === "success"
                ? "text-green-11"
                : mayaTestState() === "error"
                  ? "text-red-11"
                  : "text-gray-9"
            ...
            aria-live="polite"
          >
            {mayaTestState() === "testing" ? "Testing connection..." : mayaTestMessage() ?? "Connection status updated."}
          </div>
        </Show>

        <Show when={mayaStatusLabel() !== "Connected"}>
          <div class="text-xs text-gray-9">MAYA server connection needed to sync skills, plugins, and commands.</div>
        </Show>
      </div>
