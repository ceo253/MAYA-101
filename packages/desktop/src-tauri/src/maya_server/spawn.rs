...
pub fn resolve_maya_port() -> Result<u16, String> {
...
pub fn build_maya_args(
    ...
    let mut args = vec![
        ...
        // Always allow all origins since the MAYA server is designed to accept
...
pub fn spawn_maya_server(
    ...
    let command = match app.shell().sidecar("maya-server") {
        Ok(command) => command,
        Err(_) => app.shell().command("maya-server"),
    };

    let args = build_maya_args(
        host,
        port,
        workspace_paths,
        token,
    ...
    command
        .spawn()
        .map_err(|e| format!("Failed to start MAYA server: {e}"))
}
