...
description: Workspace guide to introduce MAYA and onboard new users.
---

# Welcome to MAYA

Hi, I'm Ben and this is MAYA. It's an open-source alternative to Claude's cowork. It helps you work on your files with AI and automate the mundane tasks so you don't have to.
...
MAYA feels like a chat app, but it can safely work with the files you allow. Put files in this workspace and I can summarize them, create new ones, or help organize them.
...
MAYA is a GUI for OpenCode. Everything that works in OpenCode works here.

Most reliable setup today:
1) Install OpenCode from opencode.ai
2) Configure providers there (models and API keys)
3) Come back to MAYA and start a session
...
End with two friendly next actions to try in MAYA."#;
...
- Reply with these four lines, exactly and in order:
  1) hey there welcome this is maya
...
fn seed_maya_agent(agent_root: &PathBuf) -> Result<(), String> {
    let agent_path = agent_root.join("maya.md");
    ...
    let doc = r#"---
description: MAYA default agent (safe, mobile-first, self-referential)
mode: primary
temperature: 0.2
---
...
When the user refers to \"you\", they mean the MAYA app and the current workspace.
...
const ENTERPRISE_ARCHIVE_URL: &str =
    "https://github.com/different-ai/maya-enterprise/archive/refs/heads/main.zip";
const ENTERPRISE_SEED_MARKER: &str = ".maya-enterprise-creators";
...
pub fn ensure_workspace_files(workspace_path: &str, preset: &str) -> Result<(), String> {
    ...
    if let Some(obj) = config.as_object_mut() {
        ...
        if current.is_empty() {
            obj.insert(
                ...
                serde_json::Value::String("maya".to_string()),
            );
            config_changed = true;
        }
    }
    ...
    let maya_path = root.join(".opencode").join("maya.json");
    if !maya_path.exists() {
        let maya = WorkspaceOpenworkConfig::new(workspace_path, preset, now_ms());

        fs::create_dir_all(maya_path.parent().unwrap())
            .map_err(|e| format!("Failed to create {}: {e}", maya_path.display()))?;

        fs::write(
            &maya_path,
            serde_json::to_string_pretty(&maya).map_err(|e| e.to_string())?,
        )
        .map_err(|e| format!("Failed to write {}: {e}", maya_path.display()))?;
    }
