...
pub fn maya_state_paths(app: &tauri::AppHandle) -> Result<(PathBuf, PathBuf), String> {
    let data_dir = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("Failed to resolve app data dir: {e}"))?;
    let file_path = data_dir.join("maya-workspaces.json");
    Ok((data_dir, file_path))
}

pub fn load_workspace_state(app: &tauri::AppHandle) -> Result<WorkspaceState, String> {
    let (_, path) = maya_state_paths(app)?;
    if !path.exists() {
...
pub fn save_workspace_state(app: &tauri::AppHandle, state: &WorkspaceState) -> Result<(), String> {
    let (dir, path) = maya_state_paths(app)?;
...
pub fn stable_workspace_id_for_maya(host_url: &str, workspace_id: Option<&str>) -> String {
    let mut key = format!("maya::{host_url}");
