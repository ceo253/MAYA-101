...
const FALLBACK_RELEASE = "https://github.com/different-ai/maya/releases";
...
export const getGithubData = async () => {
  const [repo, releases] = await Promise.all([
    fetchJson<Repo>("https://api.github.com/repos/different-ai/maya"),
    fetchJson<Release[]>(
      "https://api.github.com/repos/different-ai/maya/releases?per_page=10"
    )
  ]);
  ...
  const isStableDesktopRelease = (release: Release) => {
    ...
    return assets.some((asset) => {
      ...
      return name.startsWith("maya-desktop-");
    });
  };
  ...
  return {
    ...
    installers: {
      ...
      linux: {
        aur: "https://aur.archlinux.org/packages/maya",
