...
```bash
curl -fsSL https://raw.githubusercontent.com/different-ai/maya/dev/packages/opencode-router/install.sh | bash
```
...
- `OPENCODE_ROUTER_HEALTH_PORT` controls the port (MAYA defaults to a random free port when using `maya`).
...
- SQLite at `~/.maya/opencode-router/opencode-router.db` unless overridden.
- Config stored at `~/.maya/opencode-router/opencode-router.json` (created by `opencode-router` or `pnpm -C packages/opencode-router setup`).
