...
async function runCli(args) {
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `maya CLI failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "maya-file-session-"));
...
const mayaUrl = `http://127.0.0.1:${port}`;
...
try {
  await waitFor(`${mayaUrl}/health`);

  const workspaces = await fetchJson(`${mayaUrl}/workspaces`, {
  ...
  assert.equal(diskContent, "updated by maya cli\n");
  ...
  console.log(JSON.stringify({ ok: true, mayaUrl, workspaceId, sessionId }, null, 2));
} catch (error) {
  console.error(
    JSON.stringify(
      {
