  ...
  <body>
    <div class="wrap">
      <div class="top">
        <div class="title">
          <h1>MAYA Toy UI</h1>
          <div class="sub">Local-first host contract harness (served by maya-server)</div>
        </div>
        <div class="row">
          ...
          <div class="body">
            ...
            <div class="panel hidden" data-panel="automations">
              <div class="row">
                ...
                <span class="small">Apply schedule on host via <span class="mono">maya-agent-lab scheduler sync</span>.</span>
              </div>
            ...
            <div class="panel hidden" data-panel="config">
              ...
              <div class="small">Uploads go to <span class="mono">.opencode/maya/inbox/</span> inside the workspace.</div>

              <div class="hr"></div>

              <div class="row">
                <button class="btn" id="btn-artifacts">List artifacts</button>
                <span class="small">Downloads read from <span class="mono">.opencode/maya/outbox/</span>.</span>
              </div>
...
const STORAGE_TOKEN = "maya.toy.token";
const STORAGE_SESSION_PREFIX = "maya.toy.session.";
...
async function ensureSession(workspaceId) {
  ...
  const created = await apiFetch("/w/" + encodeURIComponent(workspaceId) + "/opencode/session", {
    method: "POST",
    body: JSON.stringify({ title: "MAYA Toy UI" }),
  });
...
async function main() {
  ...
  qs("#btn-delete-workspace").onclick = async () => {
    if (!confirm("Delete this workspace from the host's MAYA server config?")) return;
    try {
