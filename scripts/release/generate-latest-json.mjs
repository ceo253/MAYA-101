...
function parseArgs(argv) {
  const options = {
    ...
    repo: process.env.GITHUB_REPOSITORY || "different-ai/maya",
    output: "latest.json",
  };
...
function updaterPlatformKeys(assetName) {
  if (!assetName.startsWith("maya-desktop-")) return [];

  const stem = assetName.slice("maya-desktop-".length);
...
function authHeaders() {
  const headers = {
    ...
    "User-Agent": "maya-release-latest-json",
  };
