...
function parseWorkspaceSelection(payload: unknown): { workspaceId: string; mayaUrl: string } | null {
  ...
  return {
    workspaceId,
    mayaUrl: `${baseUrl}/w/${encodeURIComponent(workspaceId)}`,
  }
}
...
workersRouter.post("/", asyncRoute(async (req, res) => {
  ...
  if (parsed.data.destination === "cloud") {
    const access = await requireCloudWorkerAccess({
      ...
      name: session.user.name ?? session.user.email ?? "MAYA User",
    })
...
workersRouter.post("/:id/tokens", asyncRoute(async (req, res) => {
  ...
  res.json({
    ...
    connect: connect ?? (instance?.url ? { mayaUrl: instance.url, workspaceId: null } : null),
  })
}))
