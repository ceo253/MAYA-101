...
async function provisionWorkerOnRender(input: ProvisionInput): Promise<ProvisionedInstance> {
  ...
  const startCommand = [
    ...
    "attempt=0; while [ $attempt -lt 3 ]; do attempt=$((attempt + 1)); maya serve --workspace /tmp/workspace --maya-host 0.0.0.0 --maya-port ${PORT:-10000} --opencode-host 127.0.0.1 --opencode-port 4096 --connect-host 127.0.0.1 --cors '*' --approval manual --no-opencode-router --verbose && exit 0; echo \"maya serve failed (attempt $attempt); retrying in 3s\"; sleep 3; done; exit 1",
  ].join(" && ")
  ...
  const payload = {
    ...
    serviceDetails: {
      ...
      envSpecificDetails: {
        buildCommand: `npm install -g maya-orchestrator@${env.render.workerOpenworkVersion}`,
        startCommand,
      },
    },
  }
