...
function makeReq({ accept = "", query = {}, host = "share.maya.software" } = {}) {
  return {
    query,
    headers: {
      accept,
...
test("renderBundlePage includes machine-readable metadata and escaped json script", () => {
  ...
  const html = renderBundlePage({
    id: "01TEST",
    rawJson,
    req: makeReq({ accept: "text/html", host: "share.maya.software" }),
  });

  assert.match(html, /data-maya-share="true"/);
  assert.match(html, /data-maya-bundle-type="skill"/);
  assert.match(html, /meta name="maya:bundle-id" content="01TEST"/);
  assert.match(html, /\?format=json/);
  assert.match(html, /maya:\/\/import-bundle\?/);
  assert.match(html, /ow_bundle=https%3A%2F%2Fshare\.maya\.software%2Fb%2F01TEST/);
  assert.match(html, /ow_intent=new_worker/);
  assert.match(html, /ow_source=share_service/);
  assert.match(html, /id="maya-bundle-json" type="application\/json"/);
